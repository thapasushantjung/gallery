...
export const usePanoramaSlider = () => {
  ...
  const maxScrollRef = useRef(0); // To store the correct max scroll height
  // Track UI/scroll state for robust syncing
  const isHoveringRef = useRef(false);
  const isAutoScrollingRef = useRef(false);
  const isAutoScrollEnabled = useRef(true); // Master switch for auto-scroll behavior
    ...
    const initSwiper = async () => {
      ...
      try {
        ...
        const startAutoScroll = (dir?: 1 | -1) => {
          ...
          // Don't start if max scroll is not calculated yet or if auto-scroll is manually disabled
          if (max <= 0 || !isAutoScrollEnabled.current) return;
        ...
        const scheduleResume = () => {
          ...
          resumeTO.current = window.setTimeout(() => {
            isAutoScrollEnabled.current = true; // Re-enable before starting
            startAutoScroll(lastDir.current);
          }, 700);
        };

        // A single handler for any user interaction that should pause auto-scroll
        const handleManualInteraction = () => {
          isAutoScrollEnabled.current = false; // Any interaction disables auto-scroll
          killAutoScroll();
          scheduleResume();
        };
        ...
        await setupScrollTrigger(swiperInstance, stopAuto, (scrollMax) => {
          const oldMax = maxScrollRef.current;
          maxScrollRef.current = scrollMax;

          if (isAutoScrollEnabled.current) {
            const sizeChanged = Math.abs(scrollMax - oldMax) > 1;
            // Start if not already running, or restart if the layout size has changed.
            if (scrollMax > 0 && (!isAutoScrollingRef.current || sizeChanged)) {
              startAutoScroll(lastDir.current);
            }
          }
        });
...
const setupScrollTrigger = async (swiperInstance: any, stopAuto: (() => void) | null, onRefreshCallback: (maxScroll: number) => void) => {
  try {
    const gsapMod: any = await import('gsap');
    const stMod: any = await import('gsap/ScrollTrigger');
    const gsapLocal = gsapMod.gsap || gsapMod.default || gsapMod;
    ...
    const trigger = ScrollTrigger.create({
      ...
      onRefresh: (self: any) => {
        setByPercent(0.30, true);
        const scrollerMax = (ScrollTrigger as any).maxScroll(self.scroller);
        onRefreshCallback(scrollerMax);
      },
    });
    ...
    setTimeout(refresh, 10); // A tiny delay for initial setup
